...
while j < n & get(quit,'value') == 0
   ...
   % Follow the links from the open page.
    %--------------------------------------
    % findstr = the target's url (front part)
    %--------------------------------------
   for f = findstr('http://plum0925.pixnet.net/blog/post/',page);  
      ...
      %----------------------------------
      % The addresses might duplicate, so skip #, because the website is made by frame
      %----------------------------------
      ...
      if (i == 0) & (m < n)
         ...
         U{m} = url;
         %------------------------
         % when crawled a new path, call downloadpic, input url and the order of it to create folders
         %------------------------
         downloadpic(url,m)
         hash(m) = hashfun(url);
         i = m;
